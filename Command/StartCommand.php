...
class StartCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('no-overview', 'o', InputOption::VALUE_NONE, 'Do not display overview in the end', null)
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($set->getQuestions()) {
            ...
            if (!$input->getOption('no-overview')) {
				$this->displayResults($set, $output);
			}

            $percentage = ($set->getValidNumber() / ($set->getErrorsNumber() + $set->getValidNumber())) * 100;
			$output->writeln(
				sprintf(
					'<comment>Overall results</comment>: <error>errors: %s</error> - <info>correct: %s</info> - <fg=magenta>%d%%</>',
					$set->getErrorsNumber(),
					$set->getValidNumber(),
					round($percentage, 0)
				)
